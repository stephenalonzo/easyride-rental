@if (session()->has(['message', 'subMessage']))
    <div class="w-1/3 fixed top-0 left-1/2 transform -translate-x-1/2 flex items-center justify-center p-4">
        <div x-data={show:true} x-init="setTimeout(() => show = false, 3000)" x-show="show" role="alert"
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    <strong class="font-medium text-gray-900">{{ session('message') }}</strong>
                    <p class="mt-0.5 text-sm text-gray-700">{{ session('subMessage') }}</p>
                </div>
                <button
